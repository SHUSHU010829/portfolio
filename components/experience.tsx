...
function ExperienceCard({
  ...
  return (
    <div className="flex flex-col gap-4 border border-border bg-bg-elevated p-6">
      {/* Company name row */}
      <div className="flex min-w-0 items-center gap-3">
        ...
        {exp.companyUrl ? (
          ...
          <span className="min-w-0 font-mono font-semibold text-fg break-words">
            {displayCompany}
          </span>
        )}
            ...
            <div className="ml-[44px] flex flex-col gap-3 border-l-2 border-accent pl-4">
              ...
              {exp.highlights.map((highlight, i) => (
                <div key={i} className="flex items-start gap-2">
                  ...
                  <p className="font-mono text-sm text-fg-muted">{highlight}</p>
                </div>
              ))}
            </div>
          </motion.div>
